...
int main(int argc, char** argv)
{
	...
	Tools::Random rr;
	Sketches::CountingSample<unsigned long> csL(sampleSize, rr);
	...
	try
	{
		...
		for (unsigned long i = 1; i <= N; i++)
		{
			...
			if (i % 100 == 0)
			{
				...
				while (top > 0)
				{
					unsigned long fL = csL.getFrequency((*it).second);
					std::ostringstream ss2;
					ss2 << (*it).second << std::flush;
					unsigned long fS = csS.getFrequency(ss2.str());
		...
		Sketches::CountingSample<unsigned long> csC(data, rr);

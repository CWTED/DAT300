...
int main(int argc, char** argv)
{
	...
	std::map<std::string, unsigned long> exact;
	std::map<std::string, unsigned long>::iterator itEx;
	...
	try
	{
		for (unsigned long i = 1; i <= N; i++)
		{
			...
			if (i % 100 == 0)
			{
				...
				m = lc.getFrequent(static_cast<unsigned long>(theta * i));
		...
		std::cerr
			...
			<< 2 * maxSketchSize << std::endl
				// every entry is two unsigned longs, count and delta.

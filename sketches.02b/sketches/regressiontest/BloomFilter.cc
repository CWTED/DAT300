...
int main(int argc, char** argv)
{
	...
	Sketches::BloomFilter bfU(bits, hashes, Sketches::HT_UNIVERSAL);
	Sketches::BloomFilter bfS(bits, hashes, Sketches::HT_SHA1);
	...
	try
	{
		...
		for (unsigned long i = 1; i <= N; i++)
		{
			...
			if (i % 100 == 0)
			{
				unsigned long falsePositivesU = 0;
				unsigned long falsePositivesS = 0;

				for (unsigned long j = 0; j < domainSize; j++)
				{
				...
				falsePositiveDif =
					static_cast<long>(falsePositivesU) -
					static_cast<long>(falsePositivesS);

				unsigned long bitsSetU = bfU.getNumberOfBitsSet();
				unsigned long bitsSetS = bfS.getNumberOfBitsSet();
		...
		Sketches::BloomFilter bfC2(data);
